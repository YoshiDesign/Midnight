...
#include <cstdio>
namespace aveng {
    bool UniformBuffer::init(VkRenderData& renderData, EngineDevice& engineDevice, VkUniformBufferData& uboData) {
        ...
        if (result != VK_SUCCESS) {
            std::printf("%s error: could not allocate uniform buffer via VMA (error: %i)\n", __FUNCTION__, result);
            return false;
        }
    ...
    void UniformBuffer::uploadData(VkRenderData& renderData, EngineDevice& engineDevice, VkUniformBufferData& uboData, VkUploadMatrices matrices) {
        void* data;
        VkResult result = vmaMapMemory(engineDevice.allocator(), uboData.bufferAlloc, &data);
        if (result != VK_SUCCESS) {
            std::printf("%s error: could not map uniform buffer memory (error: %i)\n", __FUNCTION__, result);
            return;
        }
    ...
    void UniformBuffer::cleanup(VkRenderData& renderData, EngineDevice& engineDevice, VkUniformBufferData& uboData) {
