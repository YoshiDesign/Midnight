...
namespace { // Note - these could be specified `inline` only to hint to the compiler, but optimization will likely do this (?)
    ...
    procgen::Triangle ensureCCW(std::span<const aveng::Vec2> pts, procgen::Triangle t) {
        aveng::Vec2 a = pts[t.A], b = pts[t.B], c = pts[t.C];
        if (orient2d(a, b, c) < 0.0) {
            // swap b,c
            return procgen::Triangle{ t.A, t.C, t.B };
        }
        return t;
    }
    ...
    procgen::TriIndex walkToPoint(
        std::pmr::vector<procgen::AdjTri>& tris,
        std::span<const aveng::Vec2> allPts,
        procgen::TriIndex startTri,
        aveng::Vec2 p
    ) {
        if (startTri < 0 || startTri >= (procgen::TriIndex)tris.size() || !tris[startTri].alive) {
            // fallback: find any alive tri
            for (procgen::TriIndex i = 0; i < (procgen::TriIndex)tris.size(); ++i) {
                if (tris[i].alive) { startTri = i; break; }
            }
            if (startTri < 0) return procgen::kInvalidTri;
        }

        procgen::TriIndex cur = startTri;

        // A conservative max-steps guard against weird degenerate cases:
        const int maxSteps = 256;

        for (int step = 0; step < maxSteps; ++step) {
            const procgen::AdjTri& t = tris[cur];
            if (!t.alive) break;

            // Ensure CCW for the containment test:
            procgen::Triangle tmp{ t.a, t.b, t.c };
            ...
            procgen::TriIndex nextTri = procgen::kInvalidTri;
            if (oAB < 0.0) nextTri = tris[cur].n2;
            else if (oBC < 0.0) nextTri = tris[cur].n0;
            else if (oCA < 0.0) nextTri = tris[cur].n1;

            if (nextTri < 0 || nextTri >= (procgen::TriIndex)tris.size() || !tris[nextTri].alive) {
    ...
    void floodFillBadTris(
        ...
        procgen::TriIndex startTri,
        aveng::Vec2 p,
        std::pmr::vector<procgen::TriIndex>& outBad // scratch allocated
    ) {
        outBad.clear();
        if (startTri < 0) return;

        // Using our scratch allocator
        std::pmr::deque<procgen::TriIndex> q(outBad.get_allocator().resource());
        std::pmr::unordered_set<procgen::TriIndex> visited(outBad.get_allocator().resource());

        q.push_back(startTri);
        visited.insert(startTri);

        while (!q.empty()) {
            procgen::TriIndex ti = q.front();
            q.pop_front();

            if (ti < 0 || ti >= (procgen::TriIndex)tris.size()) continue;
            procgen::AdjTri& t = tris[ti];
            if (!t.alive) continue;
            ...
            procgen::TriIndex neigh[3] = { t.n0, t.n1, t.n2 };
            for (procgen::TriIndex nj : neigh) {
                if (nj < 0) continue;
    ...
    uint64_t makeEdgeKey(procgen::SiteIndex u, procgen::SiteIndex v) {
        procgen::SiteIndex lo = (u < v) ? u : v;
        procgen::SiteIndex hi = (u < v) ? v : u;
        return (uint64_t(hi) << 32) | uint64_t(lo);
    }

    // Directed edge key: pack(origin,dest)
    uint64_t makeDirKey(procgen::SiteIndex o, procgen::SiteIndex d) {
    ...
    procgen::TriEdgeDesc triEdgeKey(const procgen::AdjTri& t, int slot) {
        switch (slot) {
    ...
    procgen::TriIndex& neighborSlot(procgen::AdjTri& t, int slot) {
    ...
    */
    ...
    bool validTri(const procgen::Triangulation& tri, procgen::TriIndex t) {
        return t >= 0 && static_cast<size_t>(t) < tri.size_tris;
    }
    bool validSite(const procgen::AllPoints& pts, procgen::SiteIndex s) {
        return s >= 0 && static_cast<size_t>(s) < pts.size_all_pts;
    }

    const procgen::Triangle& getTri(const procgen::Triangulation& tri, procgen::TriIndex t) {
        return tri.tris[static_cast<size_t>(t)];
    }

    // Adjust these field names to match your Triangle struct.
    procgen::SiteIndex triA(const procgen::Triangle& t) { return t.A; }
    procgen::SiteIndex triB(const procgen::Triangle& t) { return t.B; }
    procgen::SiteIndex triC(const procgen::Triangle& t) { return t.C; }

    // Adjust these field names to match your HalfEdge struct.
    procgen::SiteIndex heOrigin(const procgen::HalfEdge& e) { return e.origin; }
    procgen::EdgeIndex heTwin(const procgen::HalfEdge& e) { return e.twin; }
    procgen::EdgeIndex heNext(const procgen::HalfEdge& e) { return e.next; }
    procgen::TriIndex  heTri(const procgen::HalfEdge& e) { return e.tri; }
...
namespace procgen {
    ...
    /*
    * Good to know things:
    * - An edge's destination (proto: EdgeDest) is `halfEdges[ halfEdges[e].next ].origin;`
    * - We don't have a `prev` member. We just use delaunay traversal based on next/twin to do that
    */
    void BuildHalfEdgeMesh(
...
#ifdef M_DEBUG
        assert(out.size_tris > 0 && "[BuildHalfEdgeMesh] Missing triangulated input");
#endif
        const size_t T = out.size_tris;
        ...
        for (size_t ti = 0; ti < T; ++ti) {
            ...
            if (!vertexPos.empty()) {
                aveng::Vec2 A = vertexPos[a];
                aveng::Vec2 B = vertexPos[b];
                aveng::Vec2 C = vertexPos[c];
                ...
                out.cache[ti] = TriangleCache{ aveng::Vec2{0,0}, aveng::Vec2{0,0}, 0.0f };
                out.circumcenters[ti] = aveng::Vec2{ 0,0 };
            }
        }
    ...
    */
    ...
    Triangulation* TriangulateBowyerWatson(
        ...
        // Scratch containers
        std::pmr::vector<aveng::Vec2> allPts(scratchMr);
        ...
        aveng::Vec2 superA{ midX - 20.0f * deltaMax, midY - 1.0f * deltaMax };
        aveng::Vec2 superB{ midX,                 midY + 20.0f * deltaMax };
        aveng::Vec2 superC{ midX + 20.0f * deltaMax, midY - 1.0f * deltaMax };
    ...
    /*
    * Math Warning
    * Barycentric / SampleScalar / TriangleGradient / TriangleNormal / SlopeAngle / SlopePercent:
    * these are for treating the terrain as a piecewise-linear continuous surface.
    * (sample heights and gradients inside triangles)
    *
    *
    */
    bool Barycentric(
        const AllPoints& allPts,
        ...
        const aveng::Vec2 a = allPts.all_pts[A];
        const aveng::Vec2 b = allPts.all_pts[B];
        const aveng::Vec2 c = allPts.all_pts[C];

        const aveng::Vec2 v0 = b - a;
        const aveng::Vec2 v1 = c - a;
        const aveng::Vec2 v2 = p - a;
        ...
        outW = aveng::BaryWeights{ wa, wb, wc };
        return true;
    }
    ...
    bool SampleScalar(
        ...
        (void)allPts; // pts used via validSite checks / indexing
        if (!valuesAtSites) return false;
        if (!validTri(tri, triID)) return false;
        if (valuesCount < tri.size_siteEdge && valuesCount < valuesCount) {
            // No-op; left intentionally empty (we can’t assume siteEdge == vertexCount always)
        }
        // We actually need valuesCount >= pts.pts.size()
        if (valuesCount < allPts.size_all_pts) return false;

        aveng::BaryWeights w{};
        if (!Barycentric(allPts, tri, triID, p, w)) return false;
    ...
    bool TriangleGradient(
        ...
        const aveng::Vec2 ab = b - a;
        const aveng::Vec2 ac = c - a;
    ...
    bool TriangleNormal(
        ...
        if (!validTri(tri, triID)) { return false; }
        if (heightsCount < allPts.size_all_pts) { return false; }
        ...
        const aveng::Vec2 a2 = allPts.all_pts[A];
        const aveng::Vec2 b2 = allPts.all_pts[B];
        const aveng::Vec2 c2 = allPts.all_pts[C];

        const aveng::Vec3 a{ a2.x, heights[A], a2.y };
        const aveng::Vec3 b{ b2.x, heights[B], b2.y };
        const aveng::Vec3 c{ c2.x, heights[C], c2.y };

        const aveng::Vec3 ab = b - a;
        const aveng::Vec3 ac = c - a;

        aveng::Vec3 n = aveng::Vec3::cross(ab, ac);
        const float L = n.len();
        if (L < 1e-12f) {
            outN = aveng::Vec3{ 0.f, 1.f, 0.f };
            return false;
        }
    ...
    bool SlopeAngleRadians(
        ...
        aveng::Vec3 n{};
        if (!TriangleNormal(allPts, tri, triID, heights, heightsCount, n)) return false;
    ...
    bool SlopePercent(
        ...
        if (!TriangleGradient(allPts, tri, triID, heights, heightsCount, dhdx, dhdz)) return false;
    ...
    static EdgeIndex findAnyOutgoing(const Triangulation& tri, SiteIndex site) {
        ...
        for (size_t i = 0; i < tri.size_halfEdges; ++i) {
    ...
    static void angleSortAround(
        const aveng::Vec2& center,
        std::pmr::vector<TriIndex>& tris,
        std::pmr::vector<aveng::Vec2>& verts,
        std::pmr::memory_resource* mr
    ) {
        struct Item { TriIndex tri; aveng::Vec2 v; float a; };

        std::pmr::vector<Item> items(mr);
        items.reserve(verts.size());

        for (size_t i = 0; i < verts.size(); ++i) {
            const aveng::Vec2 d = verts[i] - center;
    ...
    VoronoiCell VoronoiCellForSite(
        ...
        EdgeIndex start = kInvalidEdge;
        if (site >= 0 && static_cast<size_t>(site) < tri.size_siteEdge) {
            start = tri.siteEdge[static_cast<size_t>(site)];
        }
        if (start == kInvalidEdge) return cell;

        // Ensure we have an outgoing edge whose origin == site (defensive)
        if (start < 0 || static_cast<size_t>(start) >= tri.size_halfEdges ||
        ...
        for (;;) {
            ...
            const HalfEdge& he = tri.halfEdges[e];
            const TriIndex ti = heTri(he);

            if (!validTri(tri, ti) || ti >= tri.size_circumcenters) {
                closed = false;
                break;
            }

            cell.tris.push_back(ti);
            cell.vertices.push_back(tri.circumcenters[ti]);

            const EdgeIndex tw = heTwin(he);
            if (tw == kInvalidEdge) { closed = false; break; }
            if (tw < 0 || tw >= tri.size_halfEdges) { closed = false; break; }

            // Move to next edge around the same site: twin.next
            const EdgeIndex next = heNext(tri.halfEdges[tw]);
            if (next < 0 || next >= tri.size_halfEdges) { closed = false; break; }

            e = next;

            // Safety: ensure we're still around the same site
            if (heOrigin(tri.halfEdges[e]) != site) {
                closed = false;
                break;
            }
        }
