...
layout(set = 1, binding = 2) uniform LightsUbo {
...
void main() {
  ...
  vec3 ambient = ambientStrength * vec3(u_Lights.ambientLightColor.rgb);

  vec3 norm = normalize(vec3(normal));
  vec3 lightDir = normalize(vec3(lightPos));

  float diff = max(dot(norm, lightDir), 0.0);
  vec3 diffuse = diff * vec3(u_Lights.ambientLightColor.rgb);
